...
router.get('/', (req, res, next) => __awaiter(this, void 0, void 0, function* () {
    ...
    try {
        ...
        for (const employee of employees) {
            const canceled = yield firebase_1.db.ref(`users/${uid}/job/canceled`).once('value').then(snap => snap.val());
            if (canceled) {
                break;
            }
            ...
            const empListFound = yield waitForElement(adminFrame, 'input[name="zAN7M"]');
            if (!empListFound) {
            ...
            const jobChangeListFound = yield waitForElement(adminFrame, 'input[name="zAN7M"]');
            ...
            for (const line of lines) {
                ...
                for (let i = 0; i < trLength; i++) {
                    ...
                    const empLookupFound = yield waitForElement(jobChangeFrame, '#z15AMMA_LKP');
                    if (!empLookupFound) {
                    ...
                    const managerListFound = yield waitForElement(empSelectFrame, 'input[name="zAMF6"]');
                    if (!managerListFound) {
...
function waitForElement(frame, selector) {
    return __awaiter(this, void 0, void 0, function* () {
        if (!frame || !frame.hasOwnProperty('$')) {
            yield helperFunctions_1.sleep(100);
        }
        let element = yield frame.$(selector);
        let found = false;
        for (let k = 0; k < 20; k++) {
            if (!element) {
                yield helperFunctions_1.sleep(100);
                element = yield frame.$(selector);
            }
            else {
                found = true;
                break;
